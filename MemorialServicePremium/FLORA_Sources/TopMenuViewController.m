...
// 色の割合を指定して生成する場合
    ...
    //枠線色
    UIColor *btncolor_bottom_line = [UIColor colorWithRed:0.67 green:0.77 blue:0.9 alpha:1.0];
...
- (IBAction)move_kumotu:(id)sender {
    ...
    NSString *_url = @"http://www.flora-g.co.jp/funeral/okuyami";
    //弔文ページをブラウザで表示
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:_url]];
}

//弔電・喪中はがきへ遷移→供花・供物へ遷移
- (IBAction)move_tyoden:(id)sender {
    
    NSString *_url = @"http://www.flora-g.co.jp/funeral/kumotsu";
    //供物ページをブラウザで表示
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:_url]];
