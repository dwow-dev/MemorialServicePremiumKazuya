...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [httpAccess deletedataKey:@"http://ms-dev.wow-d.net/cooperation/deceasedlistarraydelete" param:parameter];
    NSLog(@"家系図delete%@",userid);
...
//                NSLog(@"hozon:NG");
        ...
        if([deceasedDao insertDeceased:deceased]) {
            ...
            if ([userid length]) {
                HttpAccess *httpAccess = [HttpAccess new];
                NSString *parameter = userid;
                [httpAccess get:POINT_ADD_USER2 param:parameter];
            }
